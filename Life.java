...
public class Life {
	...
	private static JSlider timeSlider = new JSlider(JSlider.HORIZONTAL, 0,
			1000, 500);
	...
	public static void initialise() {
		...
		resetEmptyButton.setSize(RESET_EMPTY_BUTTON_WIDTH,
				RESET_EMPTY_BUTTON_HEIGHT);
		resetEmptyButton
				.setLocation(RESET_EMPTY_BUTTON_X, RESET_EMPTY_BUTTON_Y);
		...
		midiPlayer.scale = new MidiScale("G", "moll");

		// Initialisation of the rule system
		ruleSystem = RuleSystem.FEXI;
		...
		if (console) {
			...
			for (int i = 0; i < width; i++) {
				for (int j = 0; j < height; j++) {
					...
					if (pane[i][j].isAlive())
						pane[i][j].setAliveColour(aliveColour);
					else
						pane[i][j].setDeadColour(deadColour);
				}
	...
	public static void update() {
		...
		// Midi player stuff
		// if ((steps % 200) <=100) {midiPlayer.scale = new MidiScale("F",
		// "dur"); System.out.println("F dur");}
		// else midiPlayer.scale = new MidiScale("D", "moll");
		// {System.out.println("D moll");}

		int nPitch = nAliveCells; // total number of alive cells
		int mPitch = Math.abs(nAliveCells - nAliveCellsLastRound); // use
																	// difference
		if (nPitch > DIM1 * DIM2)
			nPitch = DIM1 * DIM2;
		...
		/*
		 * too much going on int newDeadRed = (deadColour.getRed()+r3) % 256;
		 * int newDeadGreen = (deadColour.getGreen()+r1) % 256; int newDeadBlue
		 * = (deadColour.getBlue()+r2) % 256; deadColour = new Color(newDeadRed,
		 * newDeadGreen, newDeadBlue);
		 */
		...
		if (steps % 50 <= 25) {
			midiPlayer.scale = new MidiScale("B", "dur");
	...
	public static void handleInput() {
		/*
		 * try{ String line = reader.readLine(); } catch(Exception
		 * emptyException){};
